...
/**
 * 自动刷新订单库存信息脚本
 * 功能：查找缺少FSOStockId.FName字段的订单，重新从金蝶系统获取完整信息并更新
 * 作者：系统自动生成
 * 创建时间：2024年
 */
...
async function processAndSaveOrder(orderData, userid) {
  try {
    ...
    if (Array.isArray(orderData)) {
      ...
      const savedOrder = await saveKingdeeOrder(salesOrderData);
      console.log('金蝶订单信息已保存到数据库:', savedOrder.order_number);
      return savedOrder;
    }
  } catch (error) {
...
async function findOrdersWithMissingStockInfo() {
  try {
    console.log('查找缺少FSOStockId.FName字段的订单...');
    
    const result = await pool.query(`
      SELECT order_number 
      FROM wecom.kingdee_orders 
      WHERE NOT (items::text LIKE '%FSOStockId.FName%') 
         OR items::text LIKE '%"FSOStockId.FName": null%' 
         OR items::text LIKE '%"FSOStockId.FName": "",%'
         OR items::text LIKE '%"FSOStockId.FName":null%'
         OR items::text LIKE '%"FSOStockId.FName":"",%'
      ORDER BY created_at DESC
    `);
    
    console.log(`找到 ${result.rows.length} 个缺少库存信息的订单`);
    return result.rows.map(row => row.order_number);
  } catch (error) {
    console.error('查询缺少库存信息的订单失败:', error.message);
    return [];
  }
}
...
async function refreshOrderStockInfo(orderNumber) {
  try {
    ...
    if (savedOrder) {
      console.log(`订单 ${orderNumber} 库存信息刷新成功`);
      return true;
    } else {
      console.log(`订单 ${orderNumber} 库存信息刷新失败`);
      return false;
    }
  } catch (error) {
